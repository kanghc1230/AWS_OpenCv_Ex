...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: # 버튼다운이 눌려지면
        for i in range(4):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        for i in range(4):
            dragSrc[i] = False # 손뗀상태

    if event == cv2.EVENT_MOUSEMOVE:
        for i in range(4):
            if dragSrc[i]: # 눌린상태면 좌표값을 업데이트하며 drawROI
                dx = x - ptOld[0]
                dy = y - ptOld[1]
...
dragSrc = [False, False, False, False] # flag

# 모서리점, 사각형 그리기 disp라는 이미지에
